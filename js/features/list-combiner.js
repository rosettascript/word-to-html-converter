...
import { logWarning, logError } from '../utils/error-handler.js';
...
export function combineLists(root, mode) {
  ...
  listPairs.reverse().forEach(({ first, second, separator }) => {
    ...
    if (first.tagName !== second.tagName) {
      ...
      logWarning(`List type mismatch: ${first.tagName} vs ${second.tagName}. Skipping combination.`);
    ...
    if (firstTypeBefore !== firstTypeAfter) {
      logError(`List type unexpectedly changed from ${firstTypeBefore} to ${firstTypeAfter}`);
